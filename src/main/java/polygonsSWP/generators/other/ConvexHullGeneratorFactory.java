...
public class ConvexHullGeneratorFactory implements PolygonGeneratorFactory {
	...
	private static class ConvexHullGenerator implements PolygonGenerator {
		...
		ConvexHullGenerator(List<Point> points, PolygonHistory steps, PolygonStatistics statistics) {
			this.points = points;
			this.steps = steps;
		...
		public Polygon generate() {
			...
			if (steps != null)
				steps.newScene().setBoundingBox(size, size)
						.addPolygon(poly, true).safe();
			return poly;
		}
